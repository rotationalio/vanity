...
func main() {
	...
	// Create vanityd command line application
	app := cli.NewApp()
	app.Name = "vanityd"
	app.Version = pkg.Version()
	app.Usage = "run and manage a vanityd server"
	app.Flags = []cli.Flag{}
	app.Commands = []*cli.Command{
		{
			Name:     "serve",
			Usage:    "run the vanityd server",
			Category: "server",
			Action:   serve,
			Flags:    []cli.Flag{},
		},
		{
			Name:     "config",
			Usage:    "print vanityd server configuration guide",
			Category: "server",
			Action:   usage,
			Flags: []cli.Flag{
				&cli.BoolFlag{
